import {HttpClient} from '@angular/common/http';
import {Injectable} from '@angular/core';
import {Book, BookDetails} from '../../models/Book';

const booksApiRoot = 'https://openlibrary.org/api/books?format=json&bibkeys=';
const getIsbnKey = (isbnId: string) => `ISBN:${isbnId}`;
const getBooksApi = (isbnKeys: string[]) =>
  booksApiRoot + isbnKeys.map(getIsbnKey).join(',');
const placeholderImageUrl =
  'data:image/png;base64,';

@Injectable({
  providedIn: 'root',
})
export class BooksService {
  books: Book[] = localStorage.books ? JSON.parse(localStorage.books) : [];

  constructor(private http: HttpClient) {}

  async fetchBooksCoverImage(
    isbnIds: string[]
  ): Promise<Record<string, string>> {
    const endpoint = getBooksApi(isbnIds);

    const data = await this.http.get<Record<string, BookDetails>>(endpoint).toPromise();
    /**
     * Will transform result into object of structure { ISBN_ID: thumbnail_url }
     */
    return Object.entries(data).reduce(
        (col, [key, value]) => ({
          ...col,
          [key]: value.thumbnail_url,
        }),
        {}
    );
  }

  async addBookCover(books: Book[]): Promise<Book[]> {
    const isbnIds = books.map((book) => book.isbn);
    const booksInfo = await this.fetchBooksCoverImage(isbnIds);
    return books.map((book) => ({
      ...book,
      thumbnail_s: booksInfo[getIsbnKey(book.isbn)] || placeholderImageUrl,
      thumbnail_m:
          (booksInfo[getIsbnKey(book.isbn)] || '').replace('-S', '-M') ||
          placeholderImageUrl,
      thumbnail_l:
          (booksInfo[getIsbnKey(book.isbn)] || '').replace('-S', '-L') ||
          placeholderImageUrl,
    }));
  }

  async fetchData(): Promise<Book[]> {
    const books = await this.http
      .get<Book[]>('https://www.anapioficeandfire.com/api/books?pageSize=100')
      .toPromise();
    this.books = await this.addBookCover(books);

    return this.books;
  }

  async getBooks(): Promise<Book[]> {
    if (!this.books.length) {
      await this.fetchData();
    }

    localStorage.books = JSON.stringify(this.books);

    return this.books;
  }

  async getBook(bookId: string | number): Promise<Book> {
    const bookIx = Number(bookId) - 1;

    if (this.books[bookIx]) {
      return this.books[bookIx];
    }

    const data = await this.http
      .get<Book>(`https://www.anapioficeandfire.com/api/books/${bookId}`)
      .toPromise();

    const [bookWithCover] = await this.addBookCover([data]);

    return bookWithCover;
  }
}
